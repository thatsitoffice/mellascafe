          ...
          <nav className="hidden lg:flex items-center space-x-8">
            {NAV_LINKS.map((link, idx) => (
              <div key={idx} className="relative group">
                ...
                {/* Simple Dropdown Hover Effect (Visual only for now) */}
                {link.hasDropdown && (
                  <div className="absolute top-full left-0 mt-2 w-48 bg-surface border border-white/10 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 shadow-xl">
                    <div className="py-2">
                      <Link href="#" className="block px-4 py-2 text-sm text-white/70 hover:text-accent hover:bg-white/5">Submenu Item 1</Link>
                      <Link href="#" className="block px-4 py-2 text-sm text-white/70 hover:text-accent hover:bg-white/5">Submenu Item 2</Link>
                    </div>
                  </div>
                )}
              </div>
